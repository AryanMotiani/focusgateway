...
<template>
  <div v-if="store.pendingApproval" class="grid min-h-screen place-items-center p-6">
    <div class="card max-w-md p-6 text-center">
      ...
      <p class="mt-2 text-sm text-muted">Click the FocusGateway icon in your browser toolbar and press <b>Allow</b> for this site. This page continues on its own.</p>
    ...
    <div v-else class="min-h-screen lg:grid lg:grid-cols-[248px_1fr]">
      ...
      <aside class="sticky top-0 hidden h-screen flex-col border-r border-line px-4 py-5 lg:flex">
        ...
        <nav class="flex flex-1 flex-col gap-0.5" aria-label="Main">
          <RouterLink v-for="n in nav" :key="n.to" :to="n.to" class="flex items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium text-muted transition hover:bg-sunk hover:text-ink"
            exact-active-class="!bg-accent-soft !text-accent">
            <Icon :name="n.icon" /> {{ n.label }}
            <span v-if="n.to === '/blocking' && activeCount" class="ml-auto rounded-full bg-accent px-2 text-[11px] font-bold text-white dark:text-[#120f24]">{{ activeCount }}</span>
        ...
        <div class="space-y-2">
          ...
          <p v-else-if="!store.state?.agent?.paired" class="px-3 text-xs text-muted">Extension active. <RouterLink to="/install" class="text-accent underline">Add the lock agent</RouterLink> for every browser.</p>
          <p v-else class="px-3 text-xs text-good">Extension + lock agent active</p>
        </div>
      </aside>

      <!-- mobile top bar -->
      <header class="sticky top-0 z-30 flex items-center justify-between border-b border-line bg-paper/90 px-4 py-3 backdrop-blur lg:hidden">
        <RouterLink to="/" class="flex items-center gap-2"><img :src="logo" alt="" class="h-7 w-7" /><span class="font-semibold">FocusGateway</span></RouterLink>
      ...
      <div v-if="menu" class="fixed inset-0 z-40 bg-paper p-4 lg:hidden" @click="menu = false">
        <div class="mb-4 flex justify-end"><button class="btn btn-ghost btn-sm" aria-label="Close menu"><Icon name="x" /></button></div>
        <RouterLink v-for="n in nav" :key="n.to" :to="n.to" class="flex items-center gap-3 rounded-xl px-3 py-3 font-medium"><Icon :name="n.icon" /> {{ n.label }}</RouterLink>
        <RouterLink to="/install" class="flex items-center gap-3 rounded-xl px-3 py-3 font-medium"><Icon name="puzzle" /> Install</RouterLink>
      </div>
      ...
      <!-- mobile bottom nav -->
      <nav class="fixed inset-x-0 bottom-0 z-30 grid grid-cols-5 border-t border-line bg-paper/95 pb-[env(safe-area-inset-bottom)] backdrop-blur lg:hidden" aria-label="Quick">
        <RouterLink v-for="n in mobileNav" :key="n.to" :to="n.to" class="flex flex-col items-center gap-0.5 py-2 text-[11px] font-medium text-muted" exact-active-class="!text-accent">
