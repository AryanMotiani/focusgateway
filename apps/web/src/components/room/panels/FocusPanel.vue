...
const side = computed(() => props.w >= clockPx.value * 3.3 + 150)
...
<template>
  <div class="flex min-h-full flex-col" :class="big && 'items-center justify-center gap-4 text-center'">
    <div class="flex gap-x-3 gap-y-1" :class="big ? 'flex-col items-center' : side ? 'items-end justify-between' : 'flex-col'">
      ...
      <p class="text-xs whitespace-nowrap text-white/60" :class="big ? 'text-sm' : side ? 'pb-1 text-right' : ''">{{ message }}</p>
    </div>
    <div v-if="store.state" class="mt-3 border-t border-white/10 pt-3" :class="big && 'w-full max-w-md text-left'">
      <FocusCard dark :compact="compact" />
    </div>
  </div>
</template>
