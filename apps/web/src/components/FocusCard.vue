...
// Colours come from the theme tokens, so it also fits the study room glass (style.css .room-ui).
  ...
  <div>
    <template v-if="compact">
      <div v-if="active && phase" class="flex items-center justify-between gap-3 text-xs">
        ...
        <button class="btn btn-sm" @click="stop">Stop early</button>
      </div>
      ...
      <div class="flex items-center gap-5">
        <div class="relative grid h-28 w-28 shrink-0 place-items-center">
          <svg viewBox="0 0 100 100" class="absolute inset-0 -rotate-90">
            <circle cx="50" cy="50" r="44" fill="none" stroke="var(--fg-line)" stroke-width="7" />
            <circle
              cx="50"
              cy="50"
              r="44"
          ...
          <span class="num text-xl">{{ countdown(phaseLeft) }}</span>
        </div>
        <div class="min-w-0">
    ...
    <template v-else>
      ...
      <div class="mt-3 grid grid-cols-3 gap-2 text-xs text-muted">
        <label>Focus min<input v-model.number="f.work" type="number" min="1" max="240" class="input mt-1" /></label>
        <label>Break min<input v-model.number="f.brk" type="number" min="0" max="60" class="input mt-1" /></label>
        <label>Rounds<input v-model.number="f.n" type="number" min="1" max="12" class="input mt-1" /></label>
      </div>
      <div class="mt-3 flex flex-wrap items-center gap-2">
        <button class="btn btn-primary" :disabled="!siteIds.length" @click="start"><Icon name="play" :size="14" /> Start focus</button>
        <button class="btn btn-ghost btn-sm" @click="picking = true">
