...
// block for example.com, opens it in a new tab, and waits for FocusGateway's blocked page to
// report back (extension/src/blocked.js sends blocking.hit).
import { ref } from 'vue'
import { TEST_DOMAIN } from '@focusgateway/core'
...
async function run() {
  ...
  try {
    ...
    const url = `https://${TEST_DOMAIN}/?focusgateway-test=${test.id}`
  ...
  <div data-blocking-test>
    ...
    <div v-else-if="status === 'failed'" role="status" class="mt-3 rounded-xl border-2 border-bad bg-bad-soft p-3 text-sm">
      ...
      <ul class="mt-2 list-disc space-y-1 pl-5 text-muted">
        <li>Private or incognito window? Allow FocusGateway there in your browser's extension settings.</li>
        <li>Firefox: open the extension's settings and switch on access to all websites.</li>
        <li>Check that the FocusGateway extension is switched on, then reload this page and try again.</li>
