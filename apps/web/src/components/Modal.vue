...
<template>
  <Teleport to="body">
    <div
      class="fixed inset-0 z-50 flex items-end justify-center bg-[#0b0918]/55 p-0 backdrop-blur-sm sm:items-center sm:p-4"
    ...
    >
      ...
      >
        <div class="mb-4 flex items-start justify-between gap-4">
          ...
          <button
            v-if="dismissable"
            class="btn-ghost -m-1 rounded-lg p-1 text-muted hover:text-ink"
            aria-label="Close"
            @click="emit('close')"
          >
            <Icon name="x" />
