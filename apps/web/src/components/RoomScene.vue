...
const props = defineProps({
  scene: { type: String, default: 'night' },
...
const buildings = (() => {
  ...
  for (let x = -40; x < 1640;) {
    ...
    for (let wy = 900 - h + 18; wy < 880; wy += 26)
      for (let wx = x + 12; wx < x + w - 14; wx += 20) if (r() < 0.32) wins.push({ x: wx, y: wy, o: 0.5 + r() * 0.5 })
...
watch(
  () => props.rain,
  (v) => !v && canvas.value?.getContext('2d').clearRect(0, 0, canvas.value.width, canvas.value.height),
)
</script>
    ...
    <svg viewBox="0 0 1600 900" preserveAspectRatio="xMidYMid slice" class="absolute inset-0 h-full w-full transition-opacity duration-700">
      <defs>
        ...
        <radialGradient id="orbGlow">
          <stop offset="0" :stop-color="s.orbGlow" />
          <stop offset="1" stop-color="transparent" />
        </radialGradient>
        <radialGradient id="lamp" cx=".15" cy="1" r=".7">
      ...
      <g>
        <g v-for="(b, i) in buildings.front" :key="'f' + i">
          ...
          <rect
            ...
            rx="1"
            :fill="s.win"
            :opacity="w.o * (scene === 'morning' ? 0.35 : 1)"
          />
        </g>
      </g>
