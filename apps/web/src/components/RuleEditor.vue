...
async function save() {
  ...
  try {
    if (!editing) {
      const tasks = f.mode === 'gated' ? newTasks.value.filter((t) => t.title.trim()).map((t) => ({ ...t, deadline: endOfToday(store.now) })) : []
      ...
      const res = await withPin('rules.update', { id: r.id, patch: payload }, { title: 'This rule is active', message: 'Editing a live rule needs your PIN.' })
  ...
  <Modal :title="editing ? 'Edit rule' : 'New blocking rule'" wide @close="emit('close')">
    <form class="space-y-5" @submit.prevent="save">
      <div v-if="!editing" class="grid gap-2 sm:grid-cols-2">
        <button type="button" class="rounded-2xl border p-4 text-left transition" :class="f.mode === 'gated' ? 'border-accent bg-accent-soft' : 'border-line hover:border-accent'" @click="f.mode = 'gated'">
          <p class="flex items-center gap-2 font-semibold"><Icon name="unlock" :size="16" /> Task-Gated window</p>
          <p class="mt-1 text-sm text-muted">Sites stay blocked during the window until your tasks for it are done. Unfinished work keeps them blocked after it ends.</p>
        </button>
        <button type="button" class="rounded-2xl border p-4 text-left transition" :class="f.mode === 'hard' ? 'border-accent bg-accent-soft' : 'border-line hover:border-accent'" @click="f.mode = 'hard'">
          <p class="flex items-center gap-2 font-semibold"><Icon name="lock" :size="16" /> Hard Block</p>
          <p class="mt-1 text-sm text-muted">Sites are blocked for the whole window, no matter what. Good for sleep, classes and exam season.</p>
        </button>
      </div>

      <div>
        <label class="label" for="r-name">Name</label>
        <input id="r-name" v-model="f.name" class="input" :placeholder="f.mode === 'hard' ? 'e.g. No phone-brain after 11' : 'e.g. Evening study'" maxlength="80" />
      </div>
      ...
      <div class="grid gap-4 sm:grid-cols-[1fr_auto]">
        ...
        <div>
          ...
          <p v-if="crossesMidnight" class="mt-1 text-xs text-muted">{{ f.start === f.end ? 'Runs a full 24 hours.' : 'Runs past midnight into the next day.' }}</p>
        </div>
      </div>
      ...
      <div v-if="f.mode === 'hard'" class="rounded-2xl border p-4" :class="f.failsafe ? 'border-line' : 'border-bad bg-bad-soft'">
        ...
        <div v-if="!f.failsafe" class="mt-3 flex gap-2 text-sm text-bad">
          ...
          <p><b>No escape hatch.</b> While this window is active there is no way to unlock, edit or delete it. Not with your PIN, not in an emergency. You can change this any time the window is not running. At your own risk.</p>
        </div>
      </div>
      ...
      <div v-if="f.mode === 'gated' && !editing" class="space-y-3">
        ...
        <div v-for="(t, i) in newTasks" :key="i" class="flex gap-2">
          <input v-model="t.title" class="input" :placeholder="i === 0 ? 'e.g. Revise chapter 5 notes' : 'Another task'" @keydown.enter.prevent="newTasks.push({ title: '', priority: 'medium' })" />
          <select v-model="t.priority" class="input w-28" aria-label="Priority"><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option></select>
        </div>
        <button type="button" class="btn btn-sm" @click="newTasks.push({ title: '', priority: 'medium' })"><Icon name="plus" :size="14" /> Another task</button>
        <p class="text-xs text-muted">New tasks are due at the end of today. Change deadlines, add subtasks or make them repeat on the Tasks page.</p>
      ...
      <div class="flex justify-end gap-2">
        ...
        <button type="submit" class="btn btn-primary" :disabled="!f.siteIds.length || !f.days.length">{{ editing ? 'Save rule' : 'Create rule' }}</button>
      </div>
    </form>
  </Modal>
</template>
