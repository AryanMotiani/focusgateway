...
const FEATURES = [
  [
    'unlock',
    'Task-gated windows',
    'Instagram stays blocked from 4 to 7 until your homework is ticked off. Not done when the window ends? It stays blocked until you are.',
  ],
  ...
  [
    'key',
    'A Failsafe that makes you think',
    'Real emergencies happen. Overriding takes your PIN, a forced wait and a typed reason, and it is logged.',
  ],
  ...
  [
    'headphones',
    'Lofi study room',
    'Rainy window, generative lofi beats and café ambience, built right in. Still plays when YouTube is blocked.',
  ],
  ...
  <div class="min-h-screen">
    <header class="mx-auto flex max-w-6xl items-center justify-between px-5 py-5">
      <div class="flex items-center gap-2.5">
        <img :src="logo" alt="" class="h-9 w-9" /><span class="text-lg font-semibold tracking-tight">FocusGateway</span>
      </div>
      <nav class="flex items-center gap-1 text-sm">
        <a :href="REPO_URL" class="btn btn-ghost btn-sm max-sm:hidden" target="_blank" rel="noopener"
    ...
    <section class="mx-auto grid max-w-6xl items-center gap-10 px-5 pt-6 pb-16 lg:grid-cols-[1.05fr_1fr] lg:pt-14">
      <div>
        ...
        <h1 class="h-display mt-4 text-6xl leading-[0.95] sm:text-7xl">
          Study first.<br /><span class="text-accent italic">Scroll later.</span>
        </h1>
        <p class="mt-5 max-w-lg text-lg text-muted">
          FocusGateway blocks distracting websites until your work is actually done. With tasks, habits and a cozy lofi study room built in.
        </p>
        ...
        <p class="mt-4 text-sm text-muted">
          Works in Chrome, Edge, Brave, Opera, Vivaldi, Arc and Firefox. Your data never leaves your computer.
        </p>
      </div>
      <RouterLink to="/room" class="group relative block aspect-[4/3] overflow-hidden rounded-[28px] shadow-2xl ring-1 ring-black/5">
        ...
        <div
          class="absolute inset-x-5 bottom-5 flex items-center justify-between rounded-2xl border border-white/10 bg-[#120f24]/60 p-4 text-white backdrop-blur-xl"
        >
          <div>
      ...
      <div class="grid gap-8 rounded-[28px] bg-[#1b1640] p-8 text-white sm:p-12 lg:grid-cols-2">
        <div>
          ...
          <p class="mt-4 text-white/70">
            Most blockers lose to "just disable the extension" or "turn on Secure DNS". Add the optional
            <b class="text-white">lock agent</b> and FocusGateway:
          </p>
        </div>
        <ul class="space-y-3 text-white/85">
          <li class="flex gap-3">
            <Icon name="check" class="mt-0.5 text-[#f6b25e]" /> Blocks your sites in every browser and app on the computer, not just one.
          </li>
          <li class="flex gap-3">
            <Icon name="check" class="mt-0.5 text-[#f6b25e]" /> Switches off Secure DNS and private windows through official browser
            policies.
          </li>
          <li class="flex gap-3">
            <Icon name="check" class="mt-0.5 text-[#f6b25e]" /> Can lock the extensions page so the blocker can’t be turned off mid-session.
          </li>
          <li class="flex gap-3">
            <Icon name="check" class="mt-0.5 text-[#f6b25e]" /> Keeps a running no-failsafe block alive even if you delete the rule.
          </li>
          <li class="flex gap-3 text-white/60">
            <Icon name="info" class="mt-0.5" /> Honest note: someone with admin rights can always undo software on their own machine.
            FocusGateway makes that slow, deliberate and visible.
          </li>
        </ul>
      </div>
    </section>
    ...
    <footer class="border-t border-line">
      <div class="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-5 py-8 text-sm text-muted">
        ...
        <div class="flex gap-4">
          <a :href="REPO_URL" target="_blank" rel="noopener" class="hover:text-ink">Source code</a
          ><RouterLink to="/install" class="hover:text-ink">Install</RouterLink
          ><RouterLink to="/room" class="hover:text-ink">Study room</RouterLink>
        </div>
      </div>
    </footer>
  </div>
