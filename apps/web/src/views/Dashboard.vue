...
import { nextWindowStart, startOfDay, addDays, computeStats, isHabitDue, isHabitDone, habitStreak, dateKey } from '@focusgateway/core'
import { store, blocks, canBlock, call, attempt } from '../lib/store.js'
import { greeting, time, date, countdown, humanDuration } from '../lib/format.js'
...
const todayTasks = computed(() =>
  s.value.tasks
    .filter((t) => !t.parentId && (t.status !== 'done' ? t.deadline < todayEnd.value || pendingIds.value.has(t.id) : t.completedAt >= startOfDay(store.now) && t.deadline < todayEnd.value))
...
<template>
  <div class="space-y-6">
    ...
    <RouterLink v-if="!canBlock" to="/install" class="flex items-center gap-3 rounded-2xl border border-warm/40 bg-warm-soft p-4 text-sm">
      ...
      <span class="flex-1"><b>Site blocking is off in this browser.</b> Tasks, habits and the study room work, and are saved on this device. Install the free extension to turn blocking on.</span>
    ...
    <section class="card overflow-hidden">
      <div v-if="blocks.blocks.length" class="divide-y divide-line">
        <div v-for="b in blocks.blocks" :key="b.ruleId || b.focusId" class="flex flex-wrap items-center gap-4 p-4 sm:p-5">
          <div class="grid h-11 w-11 place-items-center rounded-2xl" :class="b.locked ? 'bg-bad-soft text-bad' : 'bg-accent-soft text-accent'">
            <Icon :name="b.kind === 'focus' ? 'target' : 'lock'" />
          </div>
          <div class="min-w-0 flex-1">
            <p class="font-semibold">{{ b.name }} <span class="ml-1 text-xs font-medium text-muted">{{ b.kind === 'hard' ? 'Hard block' : b.kind === 'gated' ? (b.extended ? 'Window over, tasks still open' : 'Task-gated') : 'Focus' }}</span></p>
            <p class="text-sm text-muted">
              <template v-if="b.kind === 'gated'">
                <template v-if="b.pendingTaskIds.length">Finish {{ b.pendingTaskIds.map((id) => taskById(id)?.title).filter(Boolean).slice(0, 3).join(', ') }}{{ b.pendingTaskIds.length > 3 ? '…' : '' }} to unlock.</template>
          ...
          <button v-if="!b.locked && b.kind !== 'focus'" class="btn btn-ghost btn-sm text-muted" @click="failsafe = { type: 'rule', id: b.ruleId }">Failsafe</button>
      ...
      <div v-else class="flex flex-wrap items-center gap-4 p-5">
        ...
        <div class="flex-1">
          ...
          <p class="text-sm text-muted" v-if="nextUp">Next: <b>{{ nextUp.r.name }}</b> {{ date(nextUp.t) === date(store.now) ? 'today' : date(nextUp.t) }} at {{ time(nextUp.t) }}</p>
    ...
    <div class="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
      ...
      <section>
        ...
        <div class="space-y-2">
          <TaskItem v-for="t in todayTasks" :key="t.id" :task="t" @edit="editing = { task: $event }" @add-subtask="editing = { parent: $event }" />
          <button v-if="!todayTasks.length" class="card flex w-full flex-col items-center gap-2 border-dashed p-8 text-center text-muted hover:border-accent" @click="editing = {}">
      ...
      <div class="space-y-6">
        <section class="card p-5">
          <h2 class="mb-3 flex items-center justify-between font-semibold">Focus session <RouterLink to="/room" class="text-xs font-medium text-accent">Open study room →</RouterLink></h2>
          <FocusCard />
        </section>

        <section class="card p-5">
          <h2 class="mb-3 flex items-center justify-between font-semibold">Habits today <RouterLink to="/habits" class="text-xs font-medium text-accent">All habits →</RouterLink></h2>
          <div v-if="habits.length" class="space-y-1.5">
            <button v-for="h in habits" :key="h.id" class="flex w-full items-center gap-3 rounded-xl px-2 py-2 text-left hover:bg-sunk" @click="attempt(() => call('habits.toggle', { id: h.id }))">
              <span class="grid h-7 w-7 place-items-center rounded-full border-2 text-sm transition" :class="isHabitDone(s.habitLogs, h.id, store.now) ? 'border-good bg-good text-white' : 'border-line'">
          ...
          <p v-else class="text-sm text-muted">No habits scheduled today. <RouterLink to="/habits" class="text-accent underline">Add one</RouterLink></p>
        </section>

        <section class="grid grid-cols-3 gap-2 text-center">
          <div class="card p-3"><p class="text-2xl font-semibold">{{ stats.today.focusMin }}</p><p class="text-xs text-muted">focus min</p></div>
          <div class="card p-3"><p class="text-2xl font-semibold">{{ stats.today.tasksDone }}</p><p class="text-xs text-muted">tasks done</p></div>
          <div class="card p-3"><p class="text-2xl font-semibold">{{ stats.today.habitsDone }}/{{ stats.today.habitsDue }}</p><p class="text-xs text-muted">habits</p></div>
        </section>
      </div>
    </div>
