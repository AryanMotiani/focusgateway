...
- Removed the Arch (AUR) package during a no-failsafe block? pacman can't cancel a removal, so the agent copies itself to `/var/lib/focusgateway/focusgateway-agent` and keeps running from there until the block ends. `sudo focusgateway-agent recover` still works. After the block, `sudo focusgateway-agent uninstall` removes that copy too. apt, dnf and rpm simply keep the package until the block ends.
