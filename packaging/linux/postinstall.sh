...
if [ "$mode" = upgrade ]; then
  "$BIN" install --keep-settings --no-browser --no-pause || echo "Regimen: could not restart the lock agent. Run: sudo regimen-agent install" >&2
else
  "$BIN" install --keep-settings --no-pause || echo "Regimen: could not start the lock agent. Run: sudo regimen-agent install" >&2
fi
exit 0
