...
- **Privacy policy URL:** https://aryanmotiani.github.io/regimen/privacy.html (file: `apps/web/public/privacy.html`)
- **Homepage / support:** https://aryanmotiani.github.io/regimen/ and https://github.com/AryanMotiani/regimen/issues
...
> Regimen blocks the sites that pull you away, and keeps them blocked until your work is actually done.
>
...
>
> Want blocks in every browser and app, and no way around them with private windows or Secure DNS? Add the free Regimen lock agent for Windows, macOS or Linux from the website.

## Single purpose (Chrome Web Store)

> Regimen helps students focus by blocking the websites they choose during the times they choose, until their tasks are done.
...
| `alarms` | Starts and ends blocks on schedule and checks task-gated windows every 30 seconds, even when no Regimen page is open. |
| `tabs` | When a block starts, finds tabs already open on a newly blocked site and sends them to the extension's "blocked" page. After installing, finds the Regimen tab the user came from to carry their setup over. Tab contents are never read. |
...
**Remote code:** none. All code ships in the package. No `eval`, no remote scripts (CSP `script-src 'self'`). The popup and the blocked page open the Regimen website in a normal tab when it is reachable (a single `HEAD` request checks that first), and the copy of the app inside the package when it is not. The website never runs inside the extension.
...
> Regimen is a free, open-source site blocker for students (https://github.com/AryanMotiani/regimen). To test: after installing, a Regimen tab opens with a short setup (choose any PIN and skip through the steps). Then open Blocking and add a Hard block for YouTube that covers the current time. Visiting youtube.com now shows the blocked page. The extension makes no network requests except a reachability check of its own website (https://aryanmotiani.github.io/regimen/, before opening it in a tab) and an optional local helper program on 127.0.0.1:47621, which reviewers do not need. No account or login exists.
