...
4. On a copy of the app on your own computer (`localhost`), a page that says **Approve this site in the extension** means one more step: click the puzzle piece in the toolbar, then Regimen, then **Allow**. Then press **Check again**. The official website (aryanmotiani.github.io/regimen) connects by itself. If it does not, reload its tab (it was open before the extension was installed).
5. Firefox: if the Regimen icon shows a red **!**, click it and press **Grant access**.
6. Private or incognito window? Extensions do not run there unless you allow Regimen in the browser's extension settings.
7. On a phone? Browser extensions run on computers. Firefox for Android runs some extensions, but Regimen does not support it yet. Tasks, habits and the room still work.

## What the app says in each situation

| Situation | What you see |
|---|---|
| No extension, first visit to any app page (the room included) | A one-time dialog, **Site blocking needs the free extension**, with the install button for your browser (or the Install page) and **Continue without blocking**. It is remembered in `localStorage` (`regimen:no-extension-seen`), the red chips stay. |
...
- the extension was installed after the page was opened (content scripts only reach pages loaded after install, and the extension reloads a Regimen tab it recognizes, but not always),
...
|---|---|---|
| Private or incognito window | Extensions are off there by default | Allow Regimen in private windows, or install the lock agent, which disables them |
...
| Phones | Browser extensions run on computers. Firefox for Android runs some, but Regimen does not support it yet | Use a computer for blocking. Tasks, habits and the room work on phones |
...
- **Firefox fallback.** Without host access, blocked sites show the browser's plain error page instead of Regimen's page with your tasks.
- **Admin users.** Anyone with administrator rights can switch off extensions or edit the hosts file. Regimen makes giving in slow and visible, not impossible.
