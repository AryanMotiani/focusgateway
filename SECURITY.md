...
Regimen runs with broad browser permissions, and the optional lock agent runs as administrator. We take reports seriously.

## What counts as a security issue here

Besides the usual (code execution, data leaks, privilege escalation through the agent), we treat **ways to get around a block through Regimen itself** as security issues. Examples:
...
- a website other than the official Regimen app (or a local copy you allowed) reading or changing your Regimen data
...
The hosted app at https://aryanmotiani.github.io/regimen/ uses a small content script (`extension/src/bridge.js`) to reach the extension. It is trusted on three checks:

1. The manifest injects the script only on `https://aryanmotiani.github.io/regimen/*`, plus `http://localhost/*` and `http://127.0.0.1/*` for development. No other site gets it.
2. `bridge.js` checks `location` again (origin and the `/regimen/` path) before it relays a message, and only relays messages the page posted to itself.
...
Please **do not open a public issue**. Use GitHub's private reporting: go to the repository's **Security** tab and click **Report a vulnerability** (https://github.com/AryanMotiani/regimen/security/advisories/new).
